...
public class SkeletonMeshSlicer : MonoBehaviour
{
    ...
    public class SliceTarget
    {
        ...
        public void AfterProcess()
        {
            ...
            {
                if(!allRootBones[i].parent.name.Contains("test"))
                {
                    go = new GameObject("test " + UnityEngine.Random.Range(0, 10000).ToString());
                    allRootBones[i].gameObject.AddComponent<SeveredLimb>().Init(go);
                }
            }
